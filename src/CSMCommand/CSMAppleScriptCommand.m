...
@implementation CSMAppleScriptCommand

-(void)dealloc{
    [theAppleScript release];
    [super dealloc];
}

-(id)initWithScriptPath:(NSString*) aPath{
    if(self = [super initWithScriptPath:aPath]){
      theAppleScript = [[NSAppleScript alloc] initWithContentsOfURL:[NSURL fileURLWithPath:aPath] error:nil];
    }return self;
}
